...
def trainMasqueCNN(X_train, X_test, y_train, y_test, batch_size=16, epochs=80):
    ...
    criterion = nn.BCEWithLogitsLoss()  # stable numériquement
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)

    def smooth_loss_logits(L):
        # régularise la variation temporelle du masque prédit (sur logits)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            yb = yb.to(device)  # (B,F,T) float {0,1}

            optimizer.zero_grad()
            logits = model(xb)  # (B,F,T)
