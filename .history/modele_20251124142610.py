...
def train(X_train, X_test, y_train, y_test):
    batch_size = 256
    N, F, T = X_train.shape
    X_train = X_train.view(X_train.size(0), F*T)
    X_test  = X_test.view(X_test.size(0), F*T)
    y_train = y_train.view(y_train.size(0), F*T)
    y_test  = y_test.view(y_test.size(0), F*T)
