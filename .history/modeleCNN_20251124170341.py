...
class CnnDenoiser(nn.Module):
    ...
    def forward(self, X_noisy_log_norm):
        """
        X_noisy_log_norm : (B, F, T) = log(|X_noisy|^2)/90
        Sortie : log(|S_hat|^2)/90
        """
        x_in = X_noisy_log_norm
        ...
        delta = self.conv_out(x).squeeze(1)   # (B,F,T) : correction
        S_hat_log_norm = x_in + delta         # résiduel : proche du noisy
